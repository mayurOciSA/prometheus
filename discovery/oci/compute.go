...
const (
	...
	ociLabelRegion              = ociLabel + "region" // https://docs.oracle.com/en-us/iaas/Content/General/Concepts/regions.htm
	...
	ociLabelMemoryInGbs          = ociLabel + "memory_in_gbs"
...
func (c *SDConfig) UnmarshalYAML(unmarshal func(interface{}) error) error {
	*c = DefaultSDConfig
	...
	if c.Tenancy == "" {
		return errors.New("oci_compute sd configuration: tenancy_ocid is required field")
	}
	if c.Region == "" {
		return errors.New("oci_compute sd configuration: region is required field")
	}
	if c.Compartment == "" {
		c.Compartment = c.Tenancy
	}
	if c.OciCreds.Type != "InstancePrincipal" && c.OciCreds.Type != "UserPrincipal" {
		return errors.New("oci_compute sd configuration: invalid type for authentication, valid types are InstancePrincipal or UserPrincipal")
	}

	if c.TagFilters != nil && len(c.TagFilters) > 0 {
		for _, filterByTags := range c.TagFilters {
			if filterByTags.TagKey == "" || filterByTags.TagValue == "" {
				return errors.New("oci_compute sd configuration: missing tag key or value, both tagKey and tagValue must be present for filtering")
			}
		}
	}
...
func getOciConfig(cfg *SDConfig) (common.ConfigurationProvider, error) {
	if cfg.OciCreds.Type == "UserPrincipal" {
		...
		if err != nil {
			return nil, errors.Wrap(err, "Error setting up OCI Authentication Config Object, using UserPrincipal OCI IAM mechanism, using OciConfigFile")
		}
		...
		if err != nil {
			return nil, errors.Wrap(err, "Error setting up OCI Authentication Config Object, using InstancePrincipal OCI IAM mechanism")
		}
		return ociConfig, nil
	}
...
func (d *Discovery) refresh(context context.Context) ([]*targetgroup.Group, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("could not obtain list of vnic-attachments: %s", err)
	}
		...
		for _, instance := range listInstancesRes.Items {
			if d.instanceMatchesFilterTags(instance) {
				...
				labels := model.LabelSet{
					...
					ociLabelMemoryInGbs:            model.LabelValue(fmt.Sprintf("%.2f", *instance.ShapeConfig.MemoryInGBs)),
...
func (d *Discovery) getCompartmentFullName(ctx context.Context) (string, error) {
	...
	if err != nil {
		return "", errors.Wrap(err, "Error when fetching OCI tenancy name for Tenancy ocid: "+tenancyOcid)
	}

	mapFromIDToName := make(map[string]string)
	mapFromIDToName[tenancyOcid] = "Tenancy(" + *resp.Name + ")" // tenancy name
	...
	for {
		...
		if err != nil {
			return "", errors.Wrap(err, fmt.Sprintf("Could not get list of compartments %s", tenancyOcid))
		}
